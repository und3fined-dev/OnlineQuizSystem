...
function ViewAttemptsPage() {
    ...
    if(error)
    {
        return (<p className="text-center text-red mt-6">{error}</p>);
    }
    return (
        <div className="flex flex-col items-center bg-gray-100 min-h-screen">
            ...
            <div className="w-full flex justify-between mb-6 px-10">
                <button onClick={() => navigate(`/quiz`)} className="px-6 py-2 text-xl rounded-lg text-white bg-blue-500 hover:bg-blue-600">
                    Back
                </button>

                <div className="bg-gray-300 shadow-lg text-xl py-2 px-6">
                    <h2>Result: {attempts[0].BestScore}</h2>
                </div>
            </div>
